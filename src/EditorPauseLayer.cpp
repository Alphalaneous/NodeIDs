...
$register_ids(EditorPauseLayer) {
    ...
    if (auto menu = this->getChildByType<CCMenu>(1)) {
        ...
        optionsMenu->setPosition(75.f GEODE_IOS(+ geode::utils::getSafeAreaRect().x), winSize.height / 2 - 25.f + 10.f);
    ...
    if (auto menu = detachAndCreateMenu(
        ...
        menu->setPosition(100.f GEODE_IOS(+ geode::utils::getSafeAreaRect().x), winSize.height - 55.f);
        menu->updateLayout();
    }
}
