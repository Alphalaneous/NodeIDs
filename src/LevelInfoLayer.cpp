...
$register_ids(LevelInfoLayer) {
    ...
    if (auto menu = this->getChildByType<CCMenu>(1)) {
        ...
        leftSideMenu->setPosition(30.f GEODE_IOS(+ (geode::utils::getSafeAreaRect().getMinX() - 20.f)), winSize.height / 2);
        ...
        menu->setPosition(
            winSize.width - 30 GEODE_IOS(- (geode::utils::getSafeAreaRect().getMinX() - 20.f)),
            winSize.height / 2
        );
        menu->setLayout(
            ColumnLayout::create()
